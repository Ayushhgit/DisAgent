...
class LongTermMemory:
    ...
    def store_bug_fix(self, bug_id: str, description: str, solution: str,
        ...
        """Record bug and its solution (thread-safe)."""
        with self._lock:
    ...
    def store_pattern(self, pattern_name: str, implementation: str,
        ...
        """Store reusable code pattern (thread-safe)."""
        with self._lock:
    ...
    def add_learning(self, learning: str, agent_id: Optional[str] = None):
        """Record agent learning/insight (thread-safe)."""
        with self._lock:
            timestamp = datetime.now().isoformat()
    ...
    def get_architecture(self) -> str:
        """Get system architecture (thread-safe)."""
        with self._lock:
            return self.project_context.architecture if self.project_context else ""

    def get_file_structure(self) -> Dict[str, str]:
        """Get all project files (thread-safe, returns copy)."""
        with self._lock:
            if self.project_context:
                return dict(self.project_context.file_structure)
            return {}

    def update_file(self, filename: str, content: str):
        """Update file in project (thread-safe)."""
        with self._lock:
            if self.project_context:
                self.project_context.file_structure[filename] = content

    def get_tech_stack(self) -> List[str]:
        """Get project tech stack (thread-safe, returns copy)."""
        with self._lock:
            if self.project_context:
                return list(self.project_context.tech_stack)
            return []

    def get_conventions(self) -> Dict[str, str]:
        """Get project coding conventions (thread-safe, returns copy)."""
        with self._lock:
            if self.project_context:
                return dict(self.project_context.conventions)
            return {}

    def search_similar_bugs(self, description: str, limit: int = 5) -> List[Dict]:
        """Find similar bugs that were fixed (thread-safe)."""
        with self._lock:
    ...
    def get_relevant_patterns(self, task_type: str, limit: int = 5) -> List[Dict]:
        """Get code patterns relevant to task (thread-safe)."""
        with self._lock:
            relevant = []
    ...
    def get_task_history(self, agent_id: Optional[str] = None,
        ...
        """Get historical tasks (thread-safe)."""
        with self._lock:
    ...
    def get_memories_by_tags(self, tags: List[str]) -> List[MemoryEntry]:
        """Get memories matching tags (thread-safe)."""
        with self._lock:
            return [
    ...
    def export_knowledge_base(self) -> Dict[str, Any]:
        """Export all knowledge for persistence (thread-safe)."""
        with self._lock:
            return {
                ...
                "bug_database": copy.deepcopy(self.bug_database),
    ...
    def import_knowledge_base(self, data: Dict[str, Any]):
        """Import knowledge from exported data (thread-safe)."""
        with self._lock:
    ...
    def save_to_disk(self):
        ...
        with self._lock:
            ...
            # Export while holding lock, write outside if possible
            data = {
                "project_name": self.project_name,
                "project_context": self.project_context.to_dict() if self.project_context else None,
                "memories": {k: v.to_dict() for k, v in self.memories.items()},
        ...
        # Write outside lock to minimize lock time
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, default=str)

    def load_from_disk(self) -> bool:
        """Load memory from disk (thread-safe)."""
        filepath = self.storage_path / f"{self.project_name}_memory.json"
    ...
    def get_summary(self) -> str:
        """Get summary of long-term memory contents (thread-safe)."""
        with self._lock:
    ...
    def clear(self):
        """Clear all long-term memory (thread-safe, use with caution)."""
        with self._lock:
